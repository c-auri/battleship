...
describe("place", () => {
    describe("throws an error", () => {
        ...
        test("when horizontal ship is placed directly above existing ship", () => {
            const board = new Board()
            const existingShip = new Ship(4)
            const newShip = new Ship(4)
        ...
        test("when horizontal ship is placed directly below existing ship", () => {
            const board = new Board()
            const existingShip = new Ship(4)
            const newShip = new Ship(4)
            board.place(existingShip, 5, 3, "vertical")
        ...
        test("when ship is placed into diagonal neighborhood of existing ship", () => {
            const board = new Board()
            const existingShip = new Ship(4)
            const newShip = new Ship(4)
            board.place(existingShip, 2, 2, "horizontal")
...
describe("attack", () => {
    describe("throws an error", () => {
        ...
        test("when given coordinate already received an attack", () => {
            const board = new Board()
            board.attack(3, 4)
    ...
    describe("does not throw", () => {
        test("when clearing a ship on the side of the board", () => {
            const board = new Board()
            board.place(new Ship(3), 9, 0, "vertical")
            board.attack(9, 0)
            board.attack(9, 1)
            board.attack(9, 2)
        })
    })
    describe("clears surrounding water", () => {
        test("when ship is sunk", () => {
            const board = new Board()
            board.place(new Ship(3), 3, 3, "horizontal")
            board.attack(3, 3)
            board.attack(4, 3)
            board.attack(5, 3)
            expect(board.getState(2, 2)).toBe("water")
            expect(board.getState(2, 3)).toBe("water")
            expect(board.getState(2, 4)).toBe("water")
            expect(board.getState(3, 2)).toBe("water")
            expect(board.getState(3, 4)).toBe("water")
            expect(board.getState(4, 2)).toBe("water")
            expect(board.getState(4, 4)).toBe("water")
            expect(board.getState(5, 2)).toBe("water")
            expect(board.getState(5, 4)).toBe("water")
            expect(board.getState(6, 2)).toBe("water")
            expect(board.getState(6, 3)).toBe("water")
            expect(board.getState(6, 4)).toBe("water")
